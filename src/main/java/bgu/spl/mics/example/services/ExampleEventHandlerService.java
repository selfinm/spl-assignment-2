...
public class ExampleEventHandlerService extends MicroService {
    ...
    public ExampleEventHandlerService(String name, String[] args) {
        ...
        if (args.length != 1) {
            throw new IllegalArgumentException(
                    "Event Handler expecting a single argument: mbt (the number of events to answer before termination)");
        }
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "Event Handler expecting the argument mbt to be a number > 0, instead received: " + args[0]);
        }
    ...
    protected void initialize() {
        ...
        subscribeEvent(ExampleEvent.class, ev -> {
            mbt--;
            System.out.println("Event Handler " + getName() + " got a new event from " + ev.getSenderName() + "! (mbt: "
                    + mbt + ")");
